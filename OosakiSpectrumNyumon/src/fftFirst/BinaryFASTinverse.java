...
public class BinaryFASTinverse {
...
//		System.out.println();
		...
		for (int ii = 0; ii < complex.length; ii++) {
			...
			System.out.printf("%2d, %7.3f, %7.3f, %7.3f \n", ii,complex[ii].real(), complex[ii].image(), complex[ii].abs()); // �L�����f�t�[���G�W��
		}
		...
		for (int ii = 0; ii < complex.length / 2 + 1; ii++) {
			...
			System.out.printf("%2d, f:%7.3f, A:%7.3f, B:%7.3f, AMP:%7.3f, PHASE:%7.3f ,FAS:%7.3f ,Power:%7.3f\n",ii, ii / (N * deltaT), a,b,amp,phase,fas,power); // �L���t�[���G�W��
		}
		
		
		
		System.out.println();
		System.out.println("Fourier inverse transform");
...
//			int j = 1;
		...
		IND = 1; // -1:Fourie Transform 1:Inverse
		kmax = 1;
		while (kmax < N) {
			int istep = kmax * 2;
			for (int k = 1; k < kmax + 1; k++) {
				...
				for (int ii = k; ii <= N; ii += istep) {
					int jj = ii + kmax;
					Complex tmp = x[jj - 1].multiply(theta.cexp());
					x[jj - 1] = x[ii - 1].diff(tmp);
					x[ii - 1] = x[ii - 1].add(tmp);
